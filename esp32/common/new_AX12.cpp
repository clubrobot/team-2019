...
void DynamixelClass::save_error(void)
{
	Time_Counter = 0;
...
int DynamixelClass::get_error()
{
    return Error_Byte;
}

void DynamixelClass::reset(unsigned char ID)
{
...
void DynamixelClass::setID(unsigned char ID, unsigned char newID)
{    
...
void DynamixelClass::setBD(unsigned char ID, long baud)
{    
...
void DynamixelClass::move(unsigned char ID, int Position)
{
...
void DynamixelClass::moveSpeed(unsigned char ID, int Position, int Speed)
{
...
void DynamixelClass::setEndless(unsigned char ID, bool Status)
{
    if ( Status ) 
    {	
...
void DynamixelClass::turn(unsigned char ID, bool SIDE, int Speed)
{		
	if (SIDE == 0){                          // Move Left///////////////////////////
...
void DynamixelClass::moveRW(unsigned char ID, int Position)
{
    char Position_H,Position_L;
    Position_H = Position >> 8;           // 16 bits - 2 x 8 bits variables
    Position_L = Position;
...
void DynamixelClass::moveSpeedRW(unsigned char ID, int Position, int Speed)
{
    char Position_H,Position_L,Speed_H,Speed_L;
    Position_H = Position >> 8;    
    Position_L = Position;                // 16 bits - 2 x 8 bits variables
...
void DynamixelClass::torqueStatus( unsigned char ID, bool Status)
{
...
void DynamixelClass::ledStatus(unsigned char ID, bool Status)
{    
...
void DynamixelClass::setTempLimit(unsigned char ID, unsigned char Temperature)
{
...
void DynamixelClass::setVoltageLimit(unsigned char ID, unsigned char DVoltage, unsigned char UVoltage)
{
...
void DynamixelClass::setMaxTorque(unsigned char ID, int MaxTorque)
{
...
void DynamixelClass::setMaxTorqueRAM(unsigned char ID, int MaxTorque)
{
    char MaxTorque_H,MaxTorque_L;
    MaxTorque_H = MaxTorque >> 8;           // 16 bits - 2 x 8 bits variables
    MaxTorque_L = MaxTorque;
...
void DynamixelClass::setSRL(unsigned char ID, unsigned char SRL)
{    
...
void DynamixelClass::setRDT(unsigned char ID, unsigned char RDT)
{    
...
void DynamixelClass::setLEDAlarm(unsigned char ID, unsigned char LEDAlarm)
{    
...
void DynamixelClass::setShutdownAlarm(unsigned char ID, unsigned char SALARM)
{    
...
void DynamixelClass::setCMargin(unsigned char ID, unsigned char CWCMargin, unsigned char CCWCMargin)
{
...
void DynamixelClass::setCSlope(unsigned char ID, unsigned char CWCSlope, unsigned char CCWCSlope)
{
...
void DynamixelClass::setPunch(unsigned char ID, int Punch)
{
...
void DynamixelClass::lockRegister(unsigned char ID)
{    
...
void AX12::setID(unsigned char newID){
	Dynamixel.setID(m_id, newID);
}
	
void AX12::setBD(long baud){
	Dynamixel.setBD(m_id, baud);
}

void AX12::move(float Position){
	if(m_endlessMode){
		setEndlessMode(OFF);
	}
...
void AX12::moveSpeed(float Position, float speed){
	if(m_endlessMode){
		setEndlessMode(OFF);
	}
	int pos = MIN(1023,Position/300*1023);
...
void AX12::setEndlessMode(bool Status){
	m_endlessMode = Status;
	Dynamixel.setEndless(m_id, Status);
}
	
void AX12::turn(int Speed){
	if(Speed>0){
		int spd = MIN(1023, Speed);
		Dynamixel.turn(m_id, 0, spd);
	}
	else{
		int spd = max(-1023, Speed);
		Dynamixel.turn(m_id, 1, spd);
	}
}

void AX12::Nextmove(float Position){
	int pos = MIN(1023,Position/300*1023);
	Dynamixel.moveRW(m_id, pos);
}
	
void AX12::NextmoveSpeed(float Position, float Speed){
	int pos = MIN(1023, Position/300*1023);
	Dynamixel.moveSpeedRW(m_id, pos, Speed);
}
...
void AX12::setTempLimit(unsigned char Temperature){
	Dynamixel.setTempLimit(m_id, Temperature);
}
	
void AX12::setAngleLimit(float CW, float CCW){
	int CWLimit = MIN(1023, CW/300*1023);
	int CCWLimit = MIN(1023, CCW/300 *1023);
	Dynamixel.setAngleLimit(m_id, CWLimit, CCWLimit);
}

void AX12::setVoltageLimit(unsigned char DVoltage, unsigned char UVoltage){
	Dynamixel.setVoltageLimit(m_id, DVoltage*10, UVoltage*10);
}

void AX12::setMaxTorque(int MaxTorque){
	Dynamixel.setMaxTorque(m_id, MaxTorque);
}

void AX12::setMaxTorqueRAM(int MaxTorque){
	Dynamixel.setMaxTorqueRAM(m_id, MaxTorque);
}

void AX12::setSRL(unsigned char SRL){
	Dynamixel.setSRL(m_id, SRL);
}
	
void AX12::setRDT(unsigned char RDT){
	Dynamixel.setRDT(m_id, RDT);
}
	
void AX12::setLEDAlarm(unsigned char LEDAlarm){
	Dynamixel.setLEDAlarm(m_id, LEDAlarm);
}
	
void AX12::setShutdownAlarm(unsigned char SALARM){
	Dynamixel.setShutdownAlarm(m_id, SALARM);
}

void AX12::setCMargin(unsigned char CWCMargin, unsigned char CCWCMargin){
	Dynamixel.setCMargin(m_id, CWCMargin, CCWCMargin);
}
	
void AX12::setCSlope(unsigned char CWCSlope, unsigned char CCWCSlope){
	Dynamixel.setCSlope(m_id, CWCSlope, CCWCSlope);
}
	
void AX12::setPunch(int Punch){
	Dynamixel.setPunch(m_id, Punch);	
}

int AX12::moving(){
	Dynamixel.moving(m_id);
}
	
void AX12::lockRegister(){
	Dynamixel.lockRegister(m_id);
}
	
int AX12::savedMove(){
	Dynamixel.RWStatus(m_id);
}	
...
double AX12::readVoltage(){
	return Dynamixel.readVoltage(m_id)/10;
}
	
double AX12::readPosition(){
	double pos = (double)(Dynamixel.readPosition(m_id)) * 300 / 1023;
	return pos;
}

double AX12::readSpeed(){
	if(m_endlessMode){
...
int AX12::hold(bool Status){
	m_holding = Status;
	Dynamixel.torqueStatus(m_id, Status);
}
...
int AX12::led(bool Status){
	Dynamixel.ledStatus(m_id, Status);
}
