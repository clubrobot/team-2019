...
void VL6180X::configureDefault(void)
{
  ...
  writeReg(READOUT__AVERAGING_SAMPLE_PERIOD, 0xF0);
...
uint8_t VL6180X::readRangeSingle(void (*functionPointer)())
{
  if(functionPointer==NULL)
    functionPointer=delayWrapper;
  writeReg(SYSRANGE__START, 0x01);
  return readRangeContinuous(functionPointer);
}
...
// range measurement)
uint8_t VL6180X::readRangeContinuous(void (*functionPointer)())
{
  ...
  uint8_t errorCode = readReg(RESULT__RANGE_STATUS);
  if((errorCode & 0xF0) != 0xB0) {  //Check if SNR Error on result
