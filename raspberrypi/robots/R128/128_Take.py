...
if __name__ == '__main__':
    ...
    armControllerBack.prepare_to_take()
    armControllerFront.prepare_to_take()

    armControllerBack.start_pump()
    armControllerFront.start_pump()

    while not (wheeledbase.goto.end() and armControllerBack.prepare_to_take.end() and armControllerFront.prepare_to_take.end()):
        time.sleep(0.1)

    armControllerBack.take()
    armControllerFront.take()
    
    while not (armControllerFront.take.end() and armControllerBack.take.end()):
        time.sleep(0.1)

    armControllerBack.put_in_tank()
    armControllerFront.put_in_tank()

    while not (armControllerFront.put_in_tank.end() and armControllerBack.put_in_tank.end()):
        time.sleep(0.1)
