...
class tabAtoms(Actionnable):
    ...
    def __init__(self, geogebra, daughter_cards, side, log):
        ...
        self.points["Pal5"] = self.geogebra.get("Pal6"+color)
    ...
    def moving(self):
        ...
        if self.side == self.YELLOW:
            self.wheeledbase.purepursuit([self.wheeledbase.get_position()[:2], self.points["Pal3"],
        ...
        self.pushers.down()
        if self.side == self.YELLOW:
            self.wheeledbase.purepursuit([self.wheeledbase.get_position()[:2], self.points["Pal5"], self.points["Pal6"]],
